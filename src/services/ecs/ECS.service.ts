...
export class ECSService extends Service {
    ...
    setupEntity = (entity:Entity) => {
        if(entity?.components) {
            for(const key in entity.components) {
                if(this.systems[key]) {
                    this.systems[key].setupEntity(entity);//@ts-ignore
                    this.entityMap.get(key)[entity.__node.tag] = entity;//@ts-ignore
    ...
    removeEntity = (tag:string) => {
        ...
        for(const key in entity.components) {
            if (this.entityMap.get(key)) {//@ts-ignore
                delete this.entityMap.get(key)[entity.__node.tag];//@ts-ignore
                this.entityKeyMap.get(key).splice(this.entityKeyMap.get(key).indexOf(entity.__node.tag),1);
            }
            if(this.systems[key]?.remove) {//@ts-ignore
    ...
    removeSystem = (tag:string) => {
        if(this.systems[tag]?.remove) {
            for(const e in this.entityKeyMap.get(tag)) {//@ts-ignore //run the remove routine over the system entities to run any desired cleanup
