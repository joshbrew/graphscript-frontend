...
export class WSSfrontend extends Service {
    ...
    socketloader = {
        'websockets': (node: WebSocketProps & GraphNode, parent: WebSocketProps & GraphNode, graph: WSSfrontend, roots: any) => {
            ...
            if (parent && parent.type === 'socket') {
                let parentWs = this.sockets[(parent as any)._id];
    ...
    openWS = (
        ...
        let subscribe = (route:any, callback?:((res:any)=>void)|string, args?: any[], key?: string, subInput?: boolean):Promise<number> => {
            return this.subscribeToSocket(route, (this.sockets[address] as any)._id, callback, args, key, subInput) as Promise<number>;
        }
